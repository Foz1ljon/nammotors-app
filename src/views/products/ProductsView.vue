  ...
  store.items
    ...
    .filter((p) => {
      ...
      const fields = [
        ...
        p.unit,
        p.warehouse,
        p.kvt != null ? String(p.kvt) : '',
        p.rpm != null ? String(p.rpm) : '',
        String(p.quantity),
        String(p.price),
      ]
...
function logEvent(action: 'added' | 'sold' | 'updated' | 'deleted', record: { code: string; name: string }, quantity?: number) {
  if (!auth.user) return
  activityLog.record({
...
function handleSellOk() {
  ...
  logEvent('sold', record, sellQty.value)
...
const transferDestinationOptions = computed(() =>
  warehousesStore.items
    .filter((w) => w.name !== transferringRecord.value?.warehouse)
...
function handleTransferOk() {
  ...
  const existing = store.items.find(
    (p) => p.code === record.code && p.warehouse === transferDestination.value,
  )
  if (existing) {
  ...
  <div>
    ...
    <a-card :bordered="false" style="margin-top: 16px">
      <a-table :data-source="items" row-key="id" size="middle" :pagination="{ pageSize: 10 }">
        ...
        <a-table-column :title="t('products.colActions')" :width="200" fixed="right">
          <template #default="{ record }">
            <a-space>
              <a-tooltip :title="t('products.viewTooltip')">
                <a-button type="text" size="small" @click="openViewModal(record)">
                  <template #icon><EyeOutlined /></template>
                </a-button>
              </a-tooltip>
              <a-tooltip :title="t('products.restockTooltip')">
                <a-button type="text" size="small" @click="openRestockModal(record)">
                  <template #icon><PlusCircleOutlined /></template>
                </a-button>
              </a-tooltip>
              <a-tooltip :title="t('products.sellTooltip')">
                <a-button type="text" size="small" :disabled="record.quantity <= 0" @click="openSellModal(record)">
                  <template #icon><ShoppingCartOutlined /></template>
                </a-button>
              </a-tooltip>
              <a-tooltip :title="t('products.transferTooltip')">
                <a-button type="text" size="small" :disabled="record.quantity <= 0" @click="openTransferModal(record)">
        ...
        <a-form-item :label="t('products.imageLabel')">
          <div class="image-field">
            ...
            <a-upload :show-upload-list="false" :before-upload="handleImageSelect" accept="image/*">
              <a-button size="small"><template #icon><UploadOutlined /></template>{{ t('products.uploadImage') }}</a-button>
            </a-upload>
          ...
          <a-col :span="12">
            <a-form-item :label="t('products.modelLabel')">
              <a-select
                ...
                allow-clear
                :options="modelsStore.items.map((m) => ({ value: m.name, label: m.name }))"
              >
                <template #dropdownRender="{ menuNode }">
                  ...
                  <div class="inline-add" @mousedown.prevent>
                    <a-input
                      ...
                      size="small"
                      :placeholder="t('products.newModelPlaceholder')"
                      @keydown.enter.prevent="addModelInline"
                    />
          ...
          <a-col :span="12">
            <a-form-item :label="t('products.warehouseLabel')">
              <a-select
                ...
                :placeholder="t('products.warehousePlaceholder')"
                :options="warehousesStore.items.map((w) => ({ value: w.name, label: w.name }))"
              >
                <template #dropdownRender="{ menuNode }">
                  <component :is="menuNode" />
                  <a-divider style="margin: 4px 0" />
                  <div class="inline-add" @mousedown.prevent>
                    <a-input
                      v-model:value="newWarehouseName"
                      size="small"
                      :placeholder="t('products.newWarehousePlaceholder')"
                      @keydown.enter.prevent="addWarehouseInline"
                    />
    ...
    >
      <div v-if="viewingRecord" class="view-details">
        ...
        <div class="view-grid">
          <div class="view-field"><span>{{ t('products.colCode') }}</span><b>{{ viewingRecord.code }}</b></div>
          <div class="view-field"><span>{{ t('products.colModel') }}</span><b>{{ viewingRecord.model || '-' }}</b></div>
          <div class="view-field"><span>{{ t('products.colKvt') }}</span><b>{{ viewingRecord.kvt != null ? viewingRecord.kvt : '-' }}</b></div>
          <div class="view-field"><span>{{ t('products.colRpm') }}</span><b>{{ viewingRecord.rpm != null ? fmt(viewingRecord.rpm) : '-' }}</b></div>
          <div class="view-field"><span>{{ t('products.colUnit') }}</span><b>{{ viewingRecord.unit }}</b></div>
          <div class="view-field"><span>{{ t('products.colQuantity') }}</span><b>{{ fmt(viewingRecord.quantity) }}</b></div>
          <div class="view-field"><span>{{ t('products.colPrice') }}</span><b>{{ fmt(viewingRecord.price) }}</b></div>
          <div class="view-field"><span>{{ t('products.colTotal') }}</span><b>{{ fmt(viewingRecord.price * viewingRecord.quantity) }}</b></div>
          <div class="view-field"><span>{{ t('products.colMinStock') }}</span><b>{{ fmt(viewingRecord.minStock) }}</b></div>
          <div class="view-field"><span>{{ t('products.colWarehouse') }}</span><b>{{ viewingRecord.warehouse }}</b></div>
          <div class="view-field"><span>{{ t('products.updatedAtLabel') }}</span><b>{{ viewingRecord.updatedAt }}</b></div>
        </div>
      </div>
    </a-modal>

    <a-modal
      v-model:open="restockModalOpen"
      :title="t('products.restockTitle')"
      :ok-text="t('products.restockConfirm')"
      :cancel-text="t('common.cancel')"
      width="400px"
      @ok="handleRestockOk"
    >
    ...
    <a-modal
      ...
      :title="t('products.sellTitle')"
      :ok-text="t('products.sellConfirm')"
      :cancel-text="t('common.cancel')"
      width="400px"
      @ok="handleSellOk"
    >
      ...
      <a-form layout="vertical">
        <a-form-item :label="t('products.sellQuantityLabel')">
          <a-input-number
            v-model:value="sellQty"
            :min="1"
            :max="sellingRecord?.quantity"
            style="width: 100%"
            autofocus
          />
        </a-form-item>
    ...
    <a-modal
      ...
      :title="t('products.transferTitle')"
      :ok-text="t('products.transferConfirm')"
      :cancel-text="t('common.cancel')"
      width="400px"
      @ok="handleTransferOk"
    >
      ...
      <a-form layout="vertical">
        <a-form-item :label="t('products.destinationLabel')">
          <a-select
            v-model:value="transferDestination"
            :placeholder="t('products.destinationPlaceholder')"
            :options="transferDestinationOptions"
          />
        </a-form-item>
        <a-form-item :label="t('products.transferQuantityLabel')">
          <a-input-number
            v-model:value="transferQty"
            :min="1"
            :max="transferringRecord?.quantity"
            style="width: 100%"
          />
        </a-form-item>
      </a-form>
